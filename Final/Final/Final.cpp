...
void set_rooms(vector<Room>& rooms)
{
    ...
    rooms[TOWN_HALL] = Room("The place where the mayor and council meet. It has a gardian statue in front it looks like a dragon.");
    ...
    rooms[FOUNDERS_STATUE] = Room("A statue of the mayors great grandfather who took the village away from the fae with the help of his pet dragon.");
    ...
    rooms[MAYORS_HOME] = Room("The mayors home. It's a large house compared to the others in the village.");
    ...
    rooms[NORTH_MAIN_ROAD] = Room("The northern main road of the village.");
    ...
    rooms[GRAVEYARD] = Room("An old graveyard that has been long forgotten. These graves are from the time of the fae.");
    ...
    rooms[OVERGROWN_GARDEN] = Room("You think this might have been a garden at one point. There are many flowers and herbs growing here.");
    ...
    rooms[VILLAGERS_HOUSES] = Room("There are many beautiful houses made of stone and wood. The building style is tudor. The villagers seem to be friendly.");
    ...
    rooms[SMALL_VILLAGE_GARDEN] = Room("A small community garden where villagers grow many varieties of vegetables and herbs to share with their community.");
    ...
    rooms[SHOPPING_CENTER] = Room("A large area where there are many shop stalls with the sounds of people haggling and the smells of many foods.");
    ...
    rooms[APOTHECARY] = Room("A small shop with potions and herbs that are said to have magical properties.\nThe shopkeeper is a very mysterious elf.");
...
void look_around(int loc, const vector<Room>& rooms, const vector<Word>& dirs) {
    cout << "I am in " << rooms[loc].description << ".\n";

    for (int i = 0; i < DIRS; ++i) {
        if (rooms[loc].exits[i] != NONE) {
            cout << "Exits are to the: " << dirs[i].str << endl;
        }
    }
}
...
void get (int loc, int noun, vector<Noun>& nouns)
{
    ...
    if(nouns[noun].location == loc)
    {
        ...
        cout << "You now have the " << nouns[noun].word << endl;
    }
    else
    {
...
void drop (int loc, int noun, vector<Noun>& nouns)
{
    ...
    if(nouns[noun].location == NONE)
    {
        ...
        cout << "You leave the " << nouns[noun].description << " behind.\n";
    }
    else
    {
...
void use (int loc, int noun, vector<Noun>& nouns)
{
    ...
    if(noun == OLD_COIN)
    {
        cout << "You flip the coin and catch it. It's heads.\n";
    }
    else if(noun == EMPTY_ALE_BOTTLE)
    {
        cout << "You look inside the bottle for a last drop. There is nothing left, but it does have a floral smell.\n";
    }
    ...
    {
        cout << "You feel the material. It is velvety.\n";
    }
    ...
    {
        cout << "You cut a piece of bread with it. Yep it is sharp!\n";
    }
...
void examine (int loc, int noun, vector<Noun>& nouns)
{
    ...
    if(nouns[noun].location == loc)
    {
        ...
        {
            cout << "The ring looks like a wedding ring. It's gold with a small diamond. Maybe someone who sells jewelery at the town square would know who it belongs to?\n";
        }
        else if(noun == WATERING_CAN)
        {
...
void search (int loc, vector<Noun>& nouns, vector<Room>& rooms)
{
    ...
    for(int i = 0; i < NOUNS; i++)
    {
        ...
        {
            cout << "You found a " << nouns[i].word << " " << nouns[i].description << endl;
        }
    }
}
...
void exchange (int loc, int noun, vector<Noun>& nouns)
{
    ...
    if (nouns[noun].location == NONE)
    {
        ...
        {
            cout << "The guildmaster is curious about how a flyer ended up outside, but is thankful to have it be returned.\n";
        ...
        {
            cout << "The blacksmith's wife who is a jeweler laughs and says that she remembers this ruby, and she will fix its owners necklace. She thanks you."<< endl;
        ...
        {
            cout << "Ceilia is happy to have the valuable love potion returned, but she is pretty secretive about who had purchased it in the first plavce.\n";
            nouns[LOVE_POTION].location = NONE;
            nouns[EMPTY_ALE_BOTTLE].location = APOTHECARY;
        }
            nouns[noun].location = loc;
    }
    else
        ...
        if(NOUN_MATCH == CELLAR_DOOR)
        {
            if(loc == GLOWING_POND || loc == WINE_CELLAR)
            {
                if(doorIsOpen == false)
                {
                    ...
                    nouns[CELLAR_DOOR] = Noun("Cellar Door", "a hidden wine cellar door", CELLAR_DOOR);
        ...
        if(NOUN_MATCH == CELLAR_DOOR)
        {
            if(loc == GLOWING_POND || loc == WINE_CELLAR)
            {
                if(doorIsOpen == true)
                {
                    ...
                    rooms[WINE_CELLAR].exits[WEST] = NONE;
                    nouns[CELLAR_DOOR].description.clear();
                    nouns[CELLAR_DOOR] = Noun("Cellar Door", "a mysterious moss covered door", CELLAR_DOOR);
